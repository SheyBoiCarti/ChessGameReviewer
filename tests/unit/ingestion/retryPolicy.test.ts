...
function pubError(code: ExtendedErrorCode, status?: number, retryAfterMs?: number): PubApiError {
  const retryable = ['CORS_ERROR', 'TIMEOUT', 'UPSTREAM_RATE_LIMITED', 'UPSTREAM_UNAVAILABLE'].includes(code);
...
describe('executeWithRetry', () => {
  ...
  ] as [ExtendedErrorCode, number | undefined][])('retries %s/%s at most three total attempts', async (code, status) => {
  ...
  it('does not retry 500, offline, abort, schema, or size errors', async () => {
    const errors = [
      ...
      new PubApiError('UPSTREAM_UNAVAILABLE', 'Server Error', false, 500)
    ];
  ...
  it('honors readable Retry-After and aborts during the wait', async () => {
    ...
    const wait = vi.fn((_ms, signal) => new Promise<void>((_, reject) => {
