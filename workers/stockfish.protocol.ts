...
export function isEngineWorkerRequest(value: unknown): value is EngineWorkerRequest {
  if (
    !record(value) ||
    value.protocolVersion !== ENGINE_WORKER_PROTOCOL_VERSION ||
  ...
  if (value.type === 'INITIALIZE')
    return (
      (value.mode === 'threaded' || value.mode === 'single-thread') && resources(value.resources)
    );
  return (
    value.type === 'EVALUATE' &&
    ...
    Number.isInteger(value.multiPv) &&
    value.multiPv > 0
  );
}
...
function resources(value: unknown): value is EngineResources {
  return (
    ...
    value.threads >= 1 &&
    Number.isInteger(value.hashMb) &&
    typeof value.hashMb === 'number' &&
    value.hashMb >= 1
  );
}
